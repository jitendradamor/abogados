        ...
        while ($negocios_query->have_posts()) {
            ...
            <?php if ($terms && !is_wp_error($terms)) { ?>
                <div class="category">
                    ...
                    //     echo '<li>' . $term->name . '</li>'; // Output the name of the category
                    // }
                   // echo '</ul>
               echo' </div>';
            }
            ...
            // echo get_the_content($negocio_id);
            ...
            <p><?php echo $trimmed_content; ?></p>
                ...
                <div class="address">
                    ...
                    <p><b>Ubicación :</b>
                        <?php echo get_post_meta($negocio_id, 'negocios_direccion', true); ?>
                    </p>
                ...
                <ul class="info-contacto">
                    ...
                    <ul class="e-address-contacto">
                        ...
                        // Get the value of 'ver_todas_opiniones' field
                        $ver_todas_opiniones = get_post_meta($negocio_id, 'ver_todas_opiniones', true);

                        // Display the value if it exists
                        if (!empty($ver_todas_opiniones)) {
                            echo '<div class="opiniones-row"><a target="_blank" href="' . esc_url($ver_todas_opiniones) . '">' . __('Ver todas las opiniones en Google', 'mi-plugin-negocios') . '</a>
                    <p>*Las opinione son de usuarios reales que han dejado su reseña acerca del servicio en la pagina de empresa de este servicio, no generamos ningun comentario aleatorio sino que son tomadas de Google My Business para mejorar la experiencia en la toma de decisión del servicio que necesites</p></div>';
                        }
                        echo '</div>'; // Cierre de .columna3
...
function mostrar_detalles_negocio($atts)
{
    ...
    if ($negocios_querys->have_posts()) {
        while ($negocios_querys->have_posts()) {
            ...
            // Output HTML for business details
            echo '<p>' . esc_html($city) . ', ' . esc_html($state) . ' <a href="' . esc_url(get_permalink()) . '">View Details</a></p>';
            echo '<p>' . esc_html($full_state_name) . '</p>';
        }
    } else {
